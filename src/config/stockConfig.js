const STOCK_CONFIG = {
  ...
  OPTION_UNDERLYINGS: {
    ...
    SENSEX: { token: "99919000", name: "SENSEX", lotSize: 15, exchange: "BFO" },
    BANKEX: {
      token: "999920050",
      name: "BSE Bankex",
      lotSize: 15,
...
const getOptionUnderlying = (symbol) => {
  ...
  if (!underlying) {
    console.warn(
      `⚠️ Option underlying configuration not found for symbol: ${symbol.toUpperCase()}`
    );
  }
...
const searchStocks = (query) => {
  ...
  try {
    ...
    console.error(
      "Error using stock master service, falling back to static search:",
      error.message
    );
    ...
    Object.keys(STOCK_CONFIG.OPTION_UNDERLYINGS).forEach((symbol) => {
      ...
      if (
        ...
        underlying.name.toLowerCase().includes(searchTerm)
      ) {
        results.push({
...
const getTokensByExchange = () => {
  ...
  try {
    ...
    console.error(
      "Error accessing stock master service for tokens:",
      error.message
    );
    return staticTokens;
  }
